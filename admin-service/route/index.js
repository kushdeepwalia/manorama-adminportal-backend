...
router.get("/getAll", authVerifyToken, async (req, res, next) => {
  try {
    ...
    if (userCount > 0) {
      ...
      const { rows: admins, rowCount: adminCount } = await pool.query("SELECT a.id AS admin_id, a.name AS name, a.email AS email, a.phone_no AS phone_no, a.status AS profile_status, a.google_sub_id AS google_sub_id, a.profile_pic AS profile_pic, a.last_logged_in AS last_logged_in, a.tenant_id AS organization_id, o.name AS organization_name, a.created_at AS created_at, a.updated_at AS updated_at FROM mst_admin a JOIN mst_organization o ON a.tenant_id = o.tenant_id WHERE a.tenant_id = ANY($1) ORDER BY a.tenant_id", [tenantids]);
